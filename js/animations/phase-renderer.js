...
function drawProphase(W, H, p, t) {
  ...
  // Poles animate inward from just outside → final resting position at 87% of ry
  // (ry ≈ H*0.30; H*0.26 ≈ 0.87×ry → clearly inside the cell)
  const spA = Math.max(0, (p - 0.2) / 0.8);
  if (spA > 0.01) {
    const pa = ease(spA);
    const prophPoleY = H * 0.26 * pa;
    pole(cx2, cy2 - prophPoleY, pa * 0.8);
    pole(cx2, cy2 + prophPoleY, pa * 0.8);
    if (pa > 0.3) {
      for (let i = 0; i < 6; i++) {
        const pos = NUC_POS[i];
        spindle(cx2, cy2 - prophPoleY, cx2 + pos.x, cy2 + pos.y, pa * 0.3, t);
        spindle(cx2, cy2 + prophPoleY, cx2 + pos.x, cy2 + pos.y, pa * 0.3, t);
      }
    }
  }
...
function drawPrometaphase(W, H, p, t) {
  ...
  // Poles at 87% of ry (ry ≈ H*0.30 → H*0.26), safely inside the cell
  const prometaPoleY = H * 0.26;
  pole(cx2, cy2 - prometaPoleY, 1);
  pole(cx2, cy2 + prometaPoleY, 1);
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (eP > 0.2) {
      ...
      spindle(cx2, cy2 - prometaPoleY, px, py, sA, t);
      spindle(cx2, cy2 + prometaPoleY, px, py, sA, t);
    }
  }
}
...
function drawMetaphase(W, H, p, t) {
  ...
  // Poles at 87% of ry (ry ≈ H*0.30 → H*0.26), safely inside the cell
  const metaPoleY = H * .26;
  pole(cx2, cy2 - metaPoleY); pole(cx2, cy2 + metaPoleY);
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (eP > .3) {
      const sA = (0.6 * (eP - .3) / .7);
      spindle(cx2, cy2 - metaPoleY, px, py, sA, t);
      spindle(cx2, cy2 + metaPoleY, px, py, sA, t);
    }
  }
...
function drawAnaphase(W, H, p, t) {
  ...
  const poleOff = (H * .3 + elongY) * 0.90; // 90% of ry — poles stay inside the elongating cell
